...
function ContentManager() {
  ...
  const [events, setEvents] = useState([
    ...
    },
    {
      id: 3,
      date: '2026-06-15',
      titleEn: 'Youth Sunday',
      time: '11:00 AM',
      descriptionEn: 'Youth-led service'
    }
  ])
  ...
  const [roster, setRoster] = useState([
    ...
    {
      id: 4,
      weekStart: '22/06/2026',
      leader: 'Grace Tan',
      pianist: 'Michael Wong',
      reader: 'Hannah Chua'
    }
  ])
    ...
    <div className="min-h-screen bg-[#F7F5F2]">
      ...
      <header className="sticky top-0 z-30 backdrop-blur-xl bg-white/80 border-b border-[#E7E0D7]">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-[20px] font-serif text-[#2D2926] leading-none">Content Manager</h1>
              <p className="text-xs text-[#8B7E72] mt-1">Website content administration</p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-5 sm:px-8 py-8">
          ...
          <div className="grid xl:grid-cols-[1.2fr_0.8fr] gap-6">
            ...
            <div className="bg-white border border-[#E7E0D7] rounded-3xl p-7 shadow-sm">
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h2 className="text-xl font-serif text-[#2D2926]">Weekly Scripture</h2>
                  <p className="text-sm text-[#8B7E72] mt-1">Update the homepage scripture section</p>
                </div>
                <button
                  onClick={() => { setHistoryType('verse'); setIsHistoryModalOpen(true); }}
                  className="flex items-center gap-2 h-10 px-4 rounded-xl border border-[#E7E0D7] text-sm text-[#5E5247] hover:bg-[#F5F1EB] transition"
                >
                  <History size={15} />
                  History
                </button>
            ...
            <div className="bg-[#2D2926] rounded-3xl p-8 text-white relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <p className="text-sm uppercase tracking-[0.2em] text-white/60 mb-6">Weekly Verse</p>
                <p className="text-2xl leading-relaxed font-serif">“{verse.text}”</p>
                <div className="mt-8 pt-6 border-t border-white/10">
                  <p className="text-lg font-medium">{verse.reference}</p>
                  <p className="text-sm text-white/60 mt-1">{verse.theme}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'events' && (
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xl font-serif text-[#2D2926]">Upcoming Events</h2>
                <p className="text-sm text-[#8B7E72] mt-1">Manage public-facing church events</p>
              </div>
              <div className="flex items-center gap-3">
                <button
                  onClick={saveEvents}
                ...
                <button
                  ...
                  className="flex items-center gap-2 h-11 px-5 rounded-2xl border border-[#E7E0D7] bg-white hover:bg-[#F5F1EB] transition"
                >
                  <Plus size={15} />
                  Add Event
                </button>
            ...
            <div className="grid gap-4">
              {events.map((event) => (
                <div key={event.id} className="bg-white border border-[#E7E0D7] rounded-3xl p-6 hover:shadow-md transition">
                    ...
                    <div className="flex gap-5 flex-1">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <h3 className="text-lg font-medium text-[#2D2926]">{event.titleEn}</h3>
                          <ChevronRight size={15} className="text-[#B7A89A]" />
                        </div>
                        <p className="text-sm text-[#8B7E72] mb-3">{event.time}</p>
                        <p className="text-sm text-[#5E5247] leading-relaxed">{event.descriptionEn}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-2">
                      <button onClick={() => handleEditEvent(event)} className="w-10 h-10 rounded-xl border border-[#E7E0D7] flex items-center justify-center hover:bg-[#F5F1EB] transition">
                        <Pencil size={15} className="text-[#5E5247]" />
                      </button>
                      <button onClick={() => handleDeleteEvent(event.id)} className="w-10 h-10 rounded-xl border border-[#E7E0D7] flex items-center justify-center hover:bg-red-50 transition">
        ...
        {activeTab === 'roster' && (
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xl font-serif text-[#2D2926]">Worship Roster</h2>
                <p className="text-sm text-[#8B7E72] mt-1">Weekly ministry scheduling overview</p>
              </div>
              <button
            ...
            <div className="grid lg:grid-cols-2 gap-5">
              {roster.map((week) => (
                <div key={week.id} className="bg-white border border-[#E7E0D7] rounded-3xl p-6 hover:shadow-md transition">
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      <p className="text-xs uppercase tracking-[0.15em] text-[#A39284] mb-2">Worship Week</p>
                      <h3 className="text-lg font-medium text-[#2D2926]">{week.weekStart}</h3>
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => saveRosterWeek(week)}
                        className="flex items-center gap-1 h-9 px-3 rounded-xl border border-[#E7E0D7] bg-white hover:bg-[#F5F1EB] transition text-xs"
                      >
                        <Save size={13} />
                        Save
                      </button>
                      <button
                        ...
                        className="w-9 h-9 rounded-xl border border-[#E7E0D7] flex items-center justify-center hover:bg-[#F5F1EB] transition"
                      >
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between py-3 border-b border-[#F0EAE2]">
                      <span className="text-sm text-[#8B7E72]">Worship Leader</span>
                      <span className="text-sm font-medium text-[#2D2926]">{week.leader}</span>
                    </div>
                    <div className="flex items-center justify-between py-3 border-b border-[#F0EAE2]">
                      <span className="text-sm text-[#8B7E72]">Pianist</span>
                      <span className="text-sm font-medium text-[#2D2926]">{week.pianist}</span>
                    </div>
                    <div className="flex items-center justify-between py-3">
                      <span className="text-sm text-[#8B7E72]">Scripture Reader</span>
                      <span className="text-sm font-medium text-[#2D2926]">{week.reader}</span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="bg-[#F5EFE6] rounded-xl p-4 mb-6">
              <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
                ...
                <div className="flex gap-2">
                  {[1, 2, 3, 4].map(week => (
                    <button key={week} onClick={() => setSelectedWeek(week)} className={`w-10 h-10 rounded-full text-sm font-medium transition ${
                      selectedWeek === week ? 'bg-[#2D2926] text-white' : 'border border-[#EAE1D4] text-[#7A6A5E] hover:bg-[#F5EFE6]'
                    }`}>{week}</button>
                  ))}
                </div>
              </div>
            ...
            {calendarPreview && (
              <div className="space-y-5">
                ...
                <div className="border rounded-xl p-4"><h3 className="text-sm font-medium mb-2">🎵 Worship Roster</h3>{calendarPreview.roster ? (
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-sm"><div><span className="text-[#8A7A6E]">Leader:</span> {calendarPreview.roster.leader}</div><div><span className="text-[#8A7A6E]">Pianist:</span> {calendarPreview.roster.pianist}</div><div><span className="text-[#8A7A6E]">Reader:</span> {calendarPreview.roster.reader}</div></div>
                ) : <p className="text-sm text-[#8A7A6E]">No roster assigned for this week</p>}</div>
                <div className="border rounded-xl p-4"><h3 className="text-sm font-medium mb-2">📅 Events</h3>{calendarPreview.events.length > 0 ? (
                ...
                ) : <p className="text-sm text-[#8A7A6E]">No events scheduled this week</p>}</div>
              </div>
            )}
          </div>
        </div>
      ...
      {isEventModalOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl">
            <div className="flex justify-between items-center mb-5">
              <h2 className="text-xl font-serif text-[#2D2926]">{editingEvent ? 'Edit Event' : 'Add Event'}</h2>
      ...
      {isRosterModalOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl">
            <div className="flex justify-between items-center mb-5">
              <h2 className="text-xl font-serif text-[#2D2926]">Edit Worship Week</h2>
